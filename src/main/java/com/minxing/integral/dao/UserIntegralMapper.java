...
//    Integer  removeUserIntegralByUserId(UserInfos userIntegral);
    @Update("UPDATE user_infos SET integral=#{integral} WHERE user_id=#{userId}")
    Integer removeUserIntegralByUserId(UserInfos userIntegral);
     ...
     * @return
     */
    @Update("UPDATE user_infos SET merit_score=#{meritScore} WHERE user_id=#{userId}")
    Integer removeUsermByUserId(UserInfos userInfos);
     ...
     * 查询勋值和积分
     */
    @Select("SELECT integral, merit_score AS meritScore  FROM user_infos  WHERE user_id=#{userId}")
    UserInfos selectMeritByUserId(@Param("userId") Integer userId);
     ...
     * @return IntegralManagementVO
     */
    @Select("SELECT u.id,u.`name`,IFNULL(ui.integral,0) AS integral,dept.short_name AS shortName FROM users u  \n" + "LEFT JOIN  user_infos ui ON ui.user_id=u.id   \n" + "LEFT JOIN  departments dept  ON dept.id=u.dept_id   \n" + " where u.role_code = 1 and u.actived = 1 and u.deleted_at > now() and u.network_id = #{networkId} " + "ORDER BY ui.integral  DESC")
     ...
     * @return
     */
    @Update("UPDATE user_infos SET merit_score=merit_score+#{merit} WHERE user_id=#{userId};")
    Integer addMeritByUserId(@Param("userId") Integer userId, @Param("merit") Integer merit);
    ...
    List<IntegralManagementVO> queryList(@Param("networkId") String networkId, @Param("name") String name, @Param("order") Integer order, @Param("type") String type);
    ...
    class IntegralSqlBuilder {
        ...
        public String ordinaryUser(@Param("itemId") final Integer itemId, @Param("type") final String type, @Param("order") final Integer order, @Param("timeStart") final Long timeStart, @Param("timeEnd") final Long timeEnd, @Param("networkId") final String networkId, @Param("name") final String name) {
            ...
            try {
                sql.append( "SELECT u.`name`, " + "       IF (SUM(ir.integral_id=1)>0,SUM(ir.integral_id=1),0) AS 'read' ," + "       IF(SUM(ir.integral_id=2)>0,SUM(ir.integral_id=2),0)  AS 'comment'," + "       IF(SUM(ir.integral_id=1)>0,SUM(ir.integral_id=1),0)+IF(SUM(ir.integral_id=2)>0,SUM(ir.integral_id=2),0) AS count   " + "    FROM  users u  " + "    LEFT JOIN integral_record ir ON ir.user_id=u.id  " + "    WHERE u.role_code = 1 and u.actived = 1 and u.deleted_at > now() and u.network_id = #{networkId} and  u.id not IN(SELECT user_id FROM  roles WHERE role_item_id=#{itemId}) " );
        ...
        public String SpecialUser(@Param("itemId") final Integer itemId, @Param("type") final String type, @Param("order") final Integer order, @Param("timeStart") final Long timeStart, @Param("timeEnd") final Long timeEnd, @Param("networkId") final String networkId, @Param("name") final String name) {
            StringBuffer sql = new StringBuffer();
            try {
                sql.append( "SELECT u.`name`, " + "       IF (SUM(ir.integral_id=1)>0,SUM(ir.integral_id=1),0) AS 'read' ," + "       IF(SUM(ir.integral_id=2)>0,SUM(ir.integral_id=2),0)  AS 'comment'," + "       IF(SUM(ir.integral_id=3)>0,SUM(ir.integral_id=3),0)  AS 'forward'," + "       IF(SUM(ir.integral_id=3)>0,SUM(ir.integral_id=3),0)+IF(SUM(ir.integral_id=2)>0,SUM(ir.integral_id=2),0) AS count1, " + "       IF (SUM(ir.integral_id=1)>0,SUM(ir.integral_id=1),0)+IF(SUM(ir.integral_id=2)>0,SUM(ir.integral_id=2),0)+IF(SUM(ir.integral_id=3)>0,SUM(ir.integral_id=3),0) AS count2 " + "    FROM  users u  " + "    LEFT JOIN integral_record ir ON ir.user_id=u.id " + "    WHERE u.role_code = 1 and u.actived = 1 and u.deleted_at > now() and u.network_id = #{networkId} and  u.id IN(SELECT user_id FROM  roles WHERE role_item_id=#{itemId}) " );
        ...
        public String queryList(@Param("networkId") final String networkId, @Param("name") final String name, @Param("order") final Integer order, @Param("type") final String type) {
            StringBuffer sql = new StringBuffer();
            try {
                sql.append( "SELECT u.id,u.`name`,IFNULL(ui.integral,0) AS integral,ui.merit_score AS meritScore,dept.short_name AS shortName,IFNULL(ui.integral,0)+ui.merit_score AS value FROM users u  \n" + " LEFT JOIN  user_infos ui ON ui.user_id=u.id   \n" + " LEFT JOIN  departments dept  ON dept.id=u.dept_id   \n" + " where u.role_code = 1 and u.actived = 1 and u.deleted_at > now() and u.network_id = #{networkId} " );
                //判断是否有name
                if (StringUtils.isNotEmpty( name )) {
                    sql.append( " AND (u.`name` LIKE #{name} OR u.pinyin LIKE #{name}  OR u.initialism LIKE #{name} ) " );
                }
                ...
                //按勋值排序
                if (type.equals( "meritScore" )) {
                    //判断是升序还是降序
                    if (order == 1) {
                        sql.append( " ORDER BY ui.merit_score  ,u.pinyin ASC" );
                    } else if (order == 0) {
